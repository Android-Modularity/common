...
public class Task<TResult> {
    ...
    // 在指定启动任务链
    public static <TResult> Task<TResult> call(int threadMode, @NonNull CallAction<TResult> action) {
    ...
    // 在当前线程追加一个任务
    public <ThenResult> Task<ThenResult> then(@NonNull TaskAction<TResult, ThenResult> action) {
        return then(IMMEDIATE, action);
    }

    // 在指定线程追加一个任务
    public <ThenResult> Task<ThenResult> then(int threadMode, @NonNull TaskAction<TResult, ThenResult> action) {
    ...
    public Task error(@NonNull ErrorAction action) {
        return error(UI, action);
    }

    // 在指定线程进行错误处理
    public Task error(int threadMode, @NonNull ErrorAction action) {
    ...
    private void runTaskAction(Object param) {
        if (alreadyCall) {
    ...
    private void runErrorAction(Exception ex) {
        Task lastTaskNode = findLastTaskNode();
